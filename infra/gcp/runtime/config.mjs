...
export function validateConfig(input) {
  ...
  if (!input || typeof input !== 'object' || Array.isArray(input) || keys.some(k => !(k in input)) || Object.keys(input).some(k => ![...keys,'mapsApiKey','mapsStaticKey'].includes(k))) throw new Error('RUNTIME_CONFIG_INVALID');
  ...
  if(input.mapsStaticKey!==undefined && (typeof input.mapsStaticKey!=='string'||!/^[A-Za-z0-9_-]{20,200}$/.test(input.mapsStaticKey)))throw new Error('RUNTIME_MAPS_KEY_INVALID');
