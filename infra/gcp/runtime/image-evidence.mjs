...
// npm sbom rejects installed override versions against the original transitive
// semver ranges (for example gaxios -> uuid). Inventory the installed image
// tree directly so the report describes shipped code, including overrides.
...
function scan(directory){
  ...
  for(const entry of fs.readdirSync(directory,{withFileTypes:true})){
    ...
    if(entry.name.startsWith('@')){if(entry.isDirectory())scan(path);continue;}
    if(!entry.isDirectory())continue;
    const manifest=path+'/package.json';
    if(!fs.existsSync(manifest))continue;
    const pkg=JSON.parse(fs.readFileSync(manifest,'utf8'));
    if(typeof pkg.name==='string'&&typeof pkg.version==='string')found.set(pkg.name+'@'+pkg.version,{name:pkg.name,version:pkg.version});
    scan(path+'/node_modules');
  }
}
scan('node_modules');
const components=[...found.values()].sort((a,b)=>a.name.localeCompare(b.name)||a.version.localeCompare(b.version)).map(pkg=>({
  type:'library','bom-ref':'pkg:npm/'+encodeURIComponent(pkg.name)+'@'+pkg.version,
  name:pkg.name,version:pkg.version,purl:'pkg:npm/'+encodeURIComponent(pkg.name)+'@'+pkg.version}));
console.log(JSON.stringify({bomFormat:'CycloneDX',specVersion:'1.6',version:1,
...
const names = ['api','config','database','guarded-main','health','init','main','manifest','recovery','road-routing','worker'].map(name => `infra/gcp/runtime/${name}.mjs`);
...
function walk(dir){for(const entry of fs.readdirSync(dir,{withFileTypes:true})){const path=dir+'/'+entry.name;if(entry.isDirectory())walk(path);else if(entry.isFile()&&(entry.name.endsWith('.js')||entry.name.endsWith('.json')))names.push(path);else if(entry.isSymbolicLink())throw new Error('UNEXPECTED_COMPILED_SYMLINK');}}
