...
import { tenantId, routes, validateConfig, classifyFailure } from './config.mjs';
import { createRecovery } from './recovery.mjs';
import {reconcileTrackingAlerts} from '@kavaroutes/postgres-persistence';
...
export async function createRuntimeWorker(input) {
  ...
  const consumer = createOrderedConsumer(pool, { consumerName: 'projection.trip', purposeReference: 'RIDER_INTAKE',
    authorize: async input => input.tenantId === tenantId });
  ...
  async function consume(payload) {
    ...
    if (payload.tenantId !== tenantId || !routes.includes(payload.route)) throw new Error('CONSUMER_AUTHORIZATION_DENIED');
    ...
    try {
      ...
      if (payload.jobType === 'kr.realtime-signal.trip.v1' && payload.purposeReference === 'RIDER_INTAKE') {
        source = (await client.query(`SELECT t.service_date::text, 'trip'::text AS resource_kind FROM outbox.message m JOIN intake.trip_request t
          ON t.tenant_id=m.tenant_id AND t.id=m.aggregate_id WHERE m.tenant_id=$1 AND m.event_id=$2`, [tenantId, payload.eventId])).rows[0];
        ...
        source = (await client.query(`SELECT r.service_date::text, 'driver-shift'::text AS resource_kind FROM outbox.message m
          ...
          WHERE m.tenant_id=$1 AND m.event_id=$2`, [tenantId, payload.eventId])).rows[0];
        ...
        source = (await client.query(`SELECT r.service_date::text, 'run'::text AS resource_kind,
          ...
          JOIN dispatch.run r ON r.tenant_id=a.tenant_id AND r.id=a.run_id
          WHERE m.tenant_id=$1 AND m.event_id=$2 AND m.payload->>'runId'=r.id::text`, [tenantId,payload.eventId])).rows[0];
    ...
    return realtime.consume(payload, { purpose: 'DISPATCH_CONTROL', scope: { streamKind: 'DISPATCH_DAY',
      scopeReference: 'branch:synthetic-all', serviceDate: source.service_date },
  ...
  async function cycle() {
    ...
    if(Date.now()-lastTrackingEvaluation>=5000){await reconcileTrackingAlerts(pool,tenantId);lastTrackingEvaluation=Date.now();}
    ...
    for (const route of routes) {
      ...
      const reconciled = await recovery.reconcile(queue);
