...
export async function applyDispatchAssignment(client:PoolClient,tenantId:string,input:DispatchAssignmentInput) {
 ...
 const assignments=await client.query(`SELECT a.* FROM dispatch.assignment a WHERE a.tenant_id=$1 AND a.run_id=$2 AND NOT EXISTS(SELECT 1 FROM dispatch.assignment_supersession s WHERE s.tenant_id=a.tenant_id AND s.prior_assignment_id=a.id) FOR UPDATE`,[tenantId,input.runId]);
 ...
 if(!rules || !capacity || capacity.seats<rules.seats_required || capacity.wheelchair_spaces<rules.wheelchair_spaces_required)throw new PersistenceConflict("relationship","resource feasibility required");
 ...
 if(executions.some(e=>!['planned','dispatched'].includes(e.lifecycle_reference)))throw new PersistenceConflict("relationship","started execution requires recovery");
 if(prior && (await client.query("SELECT 1 FROM execution.shift_policy_snapshot WHERE tenant_id=$1 AND assignment_id=$2 AND lifecycle<>'SHIFT_ENDED' LIMIT 1",[tenantId,prior.id])).rowCount)throw new PersistenceConflict("relationship","active shift must resolve before reassignment");
...
export function createDispatchBoardReader(pool:Pool){return async(tenantId:string,serviceDate:string)=>withTenantTransaction(pool,tenantId,'kavaroutes_api',async db=>{
 ...
 const runs=(await db.query(`SELECT r.id,r.aggregate_version,r.lifecycle_reference,r.planned_start_at,r.planned_end_at,r.service_timezone,
  a.id AS assignment_id,a.driver_id,a.vehicle_id FROM dispatch.run r LEFT JOIN dispatch.assignment a ON a.tenant_id=r.tenant_id AND a.run_id=r.id AND NOT EXISTS(SELECT 1 FROM dispatch.assignment_supersession s WHERE s.tenant_id=a.tenant_id AND s.prior_assignment_id=a.id)
