...
export function createPostgresRealtimeStore(pool: Pool, codec: TestOnlyCursorCodec, options: { readonly now?: () => Date; readonly idFactory?: () => string; readonly notify?: () => Promise<void> } = {}) {
  ...
  async function consume(unsafePayload: unknown, decision: RealtimeProjectionDecision, control: { readonly failBeforeCommit?: boolean; readonly failAfterCommit?: boolean } = {}): Promise<"APPLIED" | "DUPLICATE" | "COALESCED"> {
    ...
    const allowedKinds: Readonly<Record<SubscriptionPurpose, ReadonlySet<string>>> = {
    ...
    if (!allowedKinds[decision.purpose].has(decision.delta.kind)) throw new Error("REALTIME_PROJECTION_POLICY_DENIED");
    const client = await pool.connect();
    let outcome: "APPLIED" | "DUPLICATE" | "COALESCED" = "APPLIED";
    try {
      ...
      const allowedSourcePurposes: Readonly<Record<SubscriptionPurpose, ReadonlySet<string>>> = {
      ...
      if (!allowedSourcePurposes[decision.purpose].has(message.purpose_reference)) throw new Error("REALTIME_SOURCE_PURPOSE_DENIED");
      ...
      [payload.tenantId, String(message.aggregate_type).toLowerCase().replaceAll("_", "-"), message.aggregate_id]);
      ...
      [payload.tenantId, proposedId, decision.purpose, decision.scope.streamKind, hash, shard]);
      ...
      if (existing) {
        ...
        sequence = Number(allocated.rows[0].last_sequence);
      ...
      [payload.tenantId, stream.id, target.kind, target.reference, target.version, decision.delta]);
      ...
      [payload.tenantId, payload.eventId, String(message.aggregate_type).toLowerCase().replaceAll("_", "-"), message.aggregate_id,
        Number(message.aggregate_version), stream.id, Number(stream.epoch), sequence, outcome, new Date(now().getTime() + 30 * 24 * 60 * 60 * 1000)]);
