import {
  ...
  type DriverSyntheticLocationRequest,
  type DriverSyntheticLocationReceipt,
...
export function decodeDriverItinerary(value: unknown): DriverItinerary {
...
function decodeShift(value: unknown, assignmentId: string): DriverShiftState {
  const source = object(value);
...
function decodeClosure(value: unknown, shiftReference: string): DriverClosureView {
...
function decodeLoginState(value:unknown,expectedDriver?:string){
  const source=object(value);
  if(Object.keys(source).sort().join()!=='claimedAt,driverId,lastLoginAt,loginId,status,version')throw new Error('INVALID_DRIVER_LOGIN_STATE');
...
export function createCloudDriverWebApi(baseUrl: string, fetcher: DevelopmentFetch) {
  ...
  const transport = createPrivateDevelopmentTransport({ baseUrl, persona: "driver", fetch: fetcher, browserSameOrigin });
  ...
  return Object.freeze({
    async authenticate(signal?: AbortSignal) {
      return transport.request("/v1/me", value => {
        ...
        return { driverId, organizationId: driverOrganizationId };
      }, undefined, signal);
    },
    ...
    claimLogin(request:{driverId:string;inviteCode:string;password:string}, key:string) {
      ...
      return transport.request(`${loginPrefix}/driver-logins/${request.driverId}/commands/claim`,
        value=>decodeLoginState(value,request.driverId),{body:{driverId:request.driverId,inviteCode:request.inviteCode,password:request.password},idempotencyKey:key});
    },
    ...
    verifyLogin(request:{loginId:string;password:string}, key:string) {
      ...
      return transport.request(`${loginPrefix}/driver-logins/commands/verify`,value=>decodeLoginState(value),{body:request,idempotencyKey:key});
    },
    itinerary(serviceDate: string, signal?: AbortSignal) {
      if (!/^\d{4}-\d{2}-\d{2}$/.test(serviceDate)) throw new Error("INVALID_SERVICE_DATE");
      return transport.request(`${prefix}/itineraries/${serviceDate}`, decodeDriverItinerary, undefined, signal);
    },
    ...
    shift(assignmentId: string, signal?: AbortSignal) {
      ...
      return transport.request(`${prefix}/shifts/assignments/${assignmentId}`, value => decodeShift(value, assignmentId), undefined, signal);
    },
    ...
    location(shiftReference: string, request: DriverSyntheticLocationRequest, key: string) {
      return transport.request(`${prefix}/shifts/${shiftReference}/synthetic-location-batches`, value => {
        const body = object(value);
        if (!Array.isArray(body.items) || body.items.length !== request.samples.length) throw new Error("INVALID_LOCATION_RECEIPT");
        return body as unknown as DriverSyntheticLocationReceipt;
      }, { body: request, idempotencyKey: key });
    },
