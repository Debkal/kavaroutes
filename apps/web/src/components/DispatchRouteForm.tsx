...
type LegDraft = {
  ...
  localTime: string; durationMinutes: string; appointmentLengthMinutes: string;
...
const emptyLeg = (): LegDraft => ({
  riderReference: "", pickupLabel: "", dropoffLabel: "",
  localTime: "09:00", durationMinutes: "45", appointmentLengthMinutes:"60",
  pickupRequired: true, dropoffRequired: true, mobilitySecurementRequired: false,
  recordClientDropoff: false,
});
type DropoffSort="RECENT"|"FREQUENT";
const sortDropoffs=(dropoffs:readonly ClientDropoff[],sort:DropoffSort)=>[...dropoffs].sort((a,b)=>sort==="FREQUENT"
  ? b.usageCount-a.usageCount || Date.parse(b.lastUsedAt??"1970-01-01")-Date.parse(a.lastUsedAt??"1970-01-01") || a.addressLabel.localeCompare(b.addressLabel)
  : Date.parse(b.lastUsedAt??"1970-01-01")-Date.parse(a.lastUsedAt??"1970-01-01") || b.usageCount-a.usageCount || a.addressLabel.localeCompare(b.addressLabel));
...
export function DispatchRouteForm({ api, serviceDate, onServiceDateChange, onPlanned, initialClientId=null }: {
  ...
  useEffect(()=>{
    ...
    const recent=sortDropoffs(client.dropoffAddresses,"RECENT")[0];
    setLegs([{...emptyLeg(),riderReference:client.displayName,pickupLabel:client.pickupAddress??"",dropoffLabel:recent?.addressLabel??"",recordClientDropoff:true}]);
  ...
  const build = (): CloudPlanRequest => {
    const resolved = legs.map(leg => {
      ...
      if(!Number.isInteger(appointmentLength)||appointmentLength<0||appointmentLength>1440)
        throw new Error("Appointment length must be between 0 and 1,440 minutes.");
      const start = resolveLocalServiceStart(serviceDate, leg.localTime, timezone);
      return { leg, duration, appointmentLength, start };
    });
    const legRequests = resolved.map(({ leg, duration, appointmentLength, start }) => ({
      ...
      appointmentLengthMinutes:appointmentLength,
      ...
      recordClientDropoff: !!clientId && leg.recordClientDropoff && !leg.generatedReturn,
    }));
  ...
  const syncReturns = (current: LegDraft[]): LegDraft[] => {
    ...
    const returns = withoutReturns.filter(leg => leg.tripType === "ROUND_TRIP").map(leg => ({
      ...
      localTime: returnClock(leg.localTime, leg.durationMinutes,leg.appointmentLengthMinutes),
    }));
    return [...withoutReturns, ...returns];
  };
  const returnClock = (localTime: string, durationMinutes: string,appointmentLengthMinutes:string) => {
    const [hours, minutes] = localTime.split(":").map(Number);
    const start = (hours ?? 9) * 60 + (minutes ?? 0) + (Number(durationMinutes) || 45)+(Number(appointmentLengthMinutes)||0);
  ...
  const selectedClient=clients.data?.value.clients.find(item=>item.clientId===clientId);
  const directory=sortDropoffs(selectedClient?.dropoffAddresses??[],dropoffSort);
  ...
  return <section aria-label="Enter a route for the driver">
    ...
    <label>Client <select value={clientId} disabled={busy || !!pending.current} onChange={event => {
      ...
      if(client){const recent=sortDropoffs(client.dropoffAddresses,"RECENT")[0];setLegs([{...emptyLeg(),riderReference:client.displayName,pickupLabel:client.pickupAddress??"",dropoffLabel:recent?.addressLabel??"",recordClientDropoff:true}]);}
    ...
    {selectedClient&&<label>Drop-off directory order <select value={dropoffSort} onChange={event=>setDropoffSort(event.target.value as DropoffSort)}>
      <option value="RECENT">Recently used</option><option value="FREQUENT">Most frequent</option>
    </select></label>}
    ...
    <ol className="plan-legs">{legs.map((leg, index) => <li key={index}>
      <fieldset disabled={busy || !!pending.current}>
        ...
        {!leg.generatedReturn&&selectedClient&&<label>Saved drop-off <select value={directory.some(item=>item.addressLabel===leg.dropoffLabel)?leg.dropoffLabel:""} onChange={event=>patch(index,{dropoffLabel:event.target.value,recordClientDropoff:true})}>
          <option value="">Enter a new address below</option>{directory.map(dropoff=><option key={dropoff.ordinal} value={dropoff.addressLabel}>{dropoff.addressLabel} · {dropoff.usageCount} trip{dropoff.usageCount===1?"":"s"}</option>)}
        </select></label>}
        <label>Drop-off address <input value={leg.dropoffLabel} onChange={event => patch(index, { dropoffLabel: event.target.value,recordClientDropoff:!!clientId&&!leg.generatedReturn })}/></label>
    ...
    <button disabled={busy || !!pending.current || legs.length >= MAX_LEGS} onClick={() => setLegs(current => [...current, {...emptyLeg(),recordClientDropoff:!!clientId}])}>Add a blank trip</button>
    <p className="form-hint">Saved destinations default to the client's most recently used address. New outbound destinations are added to that client's directory when the route is saved.</p>
