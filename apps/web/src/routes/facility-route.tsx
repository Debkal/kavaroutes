...
export function loader({ request }: LoaderFunctionArgs) {
  if (new URL(request.url).search !== "") throw new Response("Client route accepts no URL context", { status: 400, statusText: "Invalid client context" });
  return null;
}
...
export function Component() {
  ...
  return <main id="main-content" className="facility-page"><section className="page-title"><div><p className="eyebrow">KavaRoutes Connect · Synthetic clients</p><h1>Client arrivals</h1><p>Minimum-necessary coordination view</p></div></section>
    {result.isPending && <p role="status">Loading client trips…</p>}
    {result.isError && <p role="alert">Client view is unavailable.</p>}
    {result.data && <section className="facility-list" aria-labelledby="client-list-heading"><h2 id="client-list-heading">Scheduled trips</h2><ul>{result.data.trips.slice(0, 40).map((trip) => <li key={trip.reference}><time>{trip.scheduledTime}</time><div><strong>{trip.riderLabel}</strong><span>{trip.status.replaceAll("_", " ")} · ETA precision: 15-minute window</span></div></li>)}</ul><p className="privacy-note">This view does not request fleet positions, driver availability, breadcrumbs, billing, claims, or administration data.</p></section>}
  </main>;
}
